...
AMBIENTE DI COLLAUDO DOCUMENTALE SEPARATO DAL MOTORE TECNARIA.
...
async def api_ask(req: QuestionRequest):
    ...
    try:
        # MODALITA' DI COLLAUDO DOCUMENTALE
        # Esempio nella chat: /listino quanto costa il codice MSZ-AY25VGK?
        if q_norm.startswith("/listino"):
            document_question = question_raw[len("/listino"):].strip()
