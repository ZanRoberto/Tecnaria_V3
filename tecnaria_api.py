# tecnaria_api.py — shim UI + GET /api/ask per l'app FastAPI esistente
# Render avvia: uvicorn tecnaria_api:app --host 0.0.0.0 --port $PORT
...
from app import app  # deve esistere: app = FastAPI(...)

# Prova a prendere il router e gli schemi dall'app
try:
    from app import intent_route
except Exception:
    intent_route = None
...
# ---- /ui sempre disponibile ----
@app.get("/ui", response_class=HTMLResponse)
def ui_page():
    return HTMLResponse(content=UI_HTML)

# ---- /__routes per debug rapido ----
@app.get("/__routes")
def __routes():
    return JSONResponse({
        "routes": [
...
# ---- GET /api/ask per test da browser ----
@app.get("/api/ask")
def api_ask_get(q: str = Query("", description="Domanda")):
    if intent_route is None:
        return JSONResponse({"ok": False, "error": "intent_route non disponibile"}, status_code=500)
    t0 = time.time()
